...
export function InstrumentCard({ instrument }: { instrument: InstrumentListItem }) {
  return (
    <div className="bg-card text-card-text rounded-2xl border border-border shadow-sm overflow-hidden flex flex-col">
      <Link href={`/instruments/${instrument.registrationNumber}`} className="contents">
      ...
      <div className="px-4 pb-4">
        <Link
          ...
          className="block text-center text-sm font-semibold text-white bg-mahidol-blue rounded-xl py-2 hover:bg-mahidol-blue/90 transition-colors"
        >
          แก้ไข
        </Link>
      </div>
