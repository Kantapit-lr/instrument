...
import { AnnualPlanListProps } from "@/types/components";

export function AnnualPlanList({ plans, loading, onDelete }: AnnualPlanListProps) {
  if (loading) {
    return <p className="p-6 text-sm text-text-muted">กำลังโหลด...</p>;
  }

  if (plans.length === 0) {
    return <p className="p-6 text-sm text-text-muted">ยังไม่มีแผนรายปีในระบบ</p>;
  }
    ...
    <table className="w-full text-sm">
      <thead>
        <tr className="bg-surface-muted text-surface-muted-text text-left">
          ...
          <th className="px-6 py-3 font-medium">ผู้เสนอ</th>
      ...
      <tbody>
        {plans.map((plan) => (
          <tr key={plan.requestListId} className="border-t border-border">
            <td className="px-6 py-3 font-medium">{plan.year}</td>
            ...
            <td className="px-6 py-3 text-right space-x-3">
              <Link href={`/annual-plans/${plan.requestListId}/edit`} className="text-action-text hover:underline">
                แก้ไข
              </Link>
